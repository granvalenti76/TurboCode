...
function resultFor(handoff, text) {
    return {
        text,
        widget: { id: widget.id, props: propsFor(handoff) },
    };
}
async function transcriptMeta(context) {
...
async function readHandoffs() {
    ...
    const handoffs = await Promise.all(names
        .filter((name) => name.endsWith(".json"))
        .map(async (name) => {
        try {
            return JSON.parse(await readFile(join(dataDirectory, name), "utf8"));
        }
        catch {
            return null;
        }
    ...
    tools: [
        ...
        defineTool({
            ...
            async handler(arguments_) {
                try {
                    const handoff = JSON.parse(await readFile(fileFor(asString(arguments_.id)), "utf8"));
