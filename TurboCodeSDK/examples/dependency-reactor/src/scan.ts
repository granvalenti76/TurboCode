...
export type Evidence={path:string;line:number};
export type Node={id:string;name:string;kind:"group"|"module";files:number};
export type Edge={source:string;target:string;count:number;evidence:Evidence[]};
...
export function imports(source:string):{name:string;line:number}[] {
 ...
 let masked="",i=0;
 const hide=(end:number)=>{masked+=source.slice(i,end).replace(/[^\n]/g," ");i=end;};
 while(i<source.length){
  ...
  if(source.startsWith("/*",i)){
   ...
   while(end<source.length&&depth){
    ...
    else if(source.startsWith("*/",end)){depth--;end+=2;}else end++;
   }
   hide(end);continue;
  }
  const opening=source.slice(i).match(/^(#*)("""|")/);
  if(opening){
   const close=opening[2]+opening[1];let end=i+opening[0].length;
   while(end<source.length){
    if(source.startsWith("\\"+opening[1],end)){end+=2+opening[1].length;continue;}
    if(source.startsWith(close,end)){end+=close.length;break;}end++;
   }
   hide(Math.min(end,source.length));continue;
  }
  masked+=source[i++];
 }
 return masked.split("\n").flatMap((line,index)=>{
...
export async function scan(path:string,signal?:AbortSignal) {
 if(!isAbsolute(path))throw new Error("Use an absolute workspace path.");
 const root=await realpath(path);
 if(!(await stat(root)).isDirectory())throw new Error("Workspace must be a directory.");
 const groups=new Map<string,Node>(),modules=new Map<string,Node>(),edges=new Map<string,Edge>();
 const notes:string[]=[];let fileCount=0,visited=0,bytes=0,skipped=0,limited=false;
 ...
 async function walk(directory:string,depth:number):Promise<void>{
  ...
  for(const entry of entries.sort((a,b)=>a.name.localeCompare(b.name))){
   signal?.throwIfAborted();
   if(++visited>20000||fileCount>=2000||bytes>16*1024*1024){limited=true;return;}
   ...
   const info=await stat(full);if(info.size>512*1024){skipped++;continue;}
   let source:string;try{source=await readFile(full,"utf8")}catch{skipped++;continue}
   bytes+=info.size;fileCount++;
   ...
   if(!groups.has(id))groups.set(id,{id,name:group,kind:"group",files:0});
   groups.get(id)!.files++;
   for(const item of imports(source)){
    const target="module:"+item.name;
    if(!modules.has(target))modules.set(target,{id:target,name:item.name,kind:"module",files:0});
    const key=id+"→"+target;
    if(!edges.has(key))edges.set(key,{source:id,target,count:0,evidence:[]});
    const edge=edges.get(key)!;edge.count++;
    if(edge.evidence.length<30)edge.evidence.push({path:file,line:item.line});
   }
  }
 }
 await walk(root,0);
 if(limited)notes.push("Partial scan: file, byte, entry or depth limit reached.");
 if(skipped)notes.push(skipped+" unreadable or oversized files skipped.");
