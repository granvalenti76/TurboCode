...
await runPlugin(definePlugin({
 ...
 tools:[defineTool({
  ...
  async handler(args,ctx) {
   try {
    if(typeof args.path!=="string") throw new Error("An absolute workspace path is required.");
   ...
   } catch(error) {return {isError:true,text:error instanceof Error?error.message:"Unable to scan workspace."};}
  }
 })]
}));
