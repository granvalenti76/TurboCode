...
export type Change = {
  path: string; status: string; staged: boolean; unstaged: boolean;
  added: number; removed: number; binary: boolean; untracked: boolean;
};
...
export async function snapshot(directory: string, signal?: AbortSignal) {
  if (!isAbsolute(directory)) throw new Error("Workspace path must be absolute.");
  if (!(await stat(directory)).isDirectory()) throw new Error("Workspace path must be a directory.");
  ...
  try { root = (await git(["rev-parse", "--show-toplevel"])).trim(); }
    ...
    return { state: "no-repository", name: basename(directory), root: directory, branch: "—", commit: "", capturedAt, files: [] as Change[] };
  }
  ...
  if (!branch) branch = (await git(["symbolic-ref", "--short", "HEAD"])).trim();
  if (branch === "HEAD") branch = "detached · " + (await git(["rev-parse", "--short", "HEAD"])).trim();
  ...
  if (before !== after) throw new Error("Repository changed during inspection. Run Repo Observatory again.");
